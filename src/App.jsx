    ...
    <main className="mx-auto flex min-h-screen w-full max-w-3xl flex-col justify-center px-5 py-10 sm:py-12">
      ...
      <section className="grid gap-3 sm:gap-4">
        <button
          ...
          className="group relative overflow-hidden rounded-lg border border-yellow-300/45 bg-zinc-900 p-5 text-left text-white shadow-2xl shadow-black/30 transition hover:-translate-y-1 hover:border-yellow-200/80 sm:p-6"
        >
          <span className="absolute inset-x-0 top-0 h-px bg-yellow-300/80" />
          <p className="text-[10px] font-black uppercase tracking-[0.28em] text-yellow-200">
            Current rankings
          </p>
          <h1 className="mt-3 text-4xl font-black sm:text-5xl">Brian's Top 20</h1>
        ...
        <div className="grid gap-3 sm:grid-cols-2 sm:gap-4">
          <button
            ...
            className="group min-h-48 rounded-lg border border-cyan-300/40 bg-zinc-900 p-5 text-left text-white shadow-2xl shadow-black/30 transition hover:-translate-y-1 hover:border-cyan-200/80 sm:p-6"
          >
          ...
          <button
            ...
            className="group min-h-48 rounded-lg border border-white/10 bg-zinc-900 p-5 text-left text-white shadow-2xl shadow-black/30 transition hover:-translate-y-1 hover:border-white/30 sm:p-6"
          >
            <p className="text-[10px] font-black uppercase tracking-[0.28em] text-zinc-400">
              Original
            </p>
            <h2 className="mt-3 text-3xl font-black sm:text-4xl">FINE Calculator</h2>
            <p className="mt-2 text-sm leading-6 text-zinc-300 sm:text-base">
...
function ScoreboardPage({ navigate }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (adminAllowed && entryRows?.length) {
        ...
        setHistoryLogs(
          (logRows || []).reduce((logsByEntry, log) => {
            return {
              ...logsByEntry,
              [log.entry_id]: [...(logsByEntry[log.entry_id] || []), log],
            };
          }, {})
        );
      }
                  ...
                  {isAdmin && latestLog && historyEntryId !== entry.id && !adjustmentOpen && (
                    <div className="mt-2 text-xs leading-5 text-zinc-400">
                      <span className="font-black text-zinc-300">
                        Last adjustment: {Number(latestLog.delta) > 0 ? `+${latestLog.delta}` : latestLog.delta}
                      </span>
                      {latestLog.note && <span> - {latestLog.note}</span>}
                    </div>
                  )}
